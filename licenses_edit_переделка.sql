  ...
  with licenses_edit as (
    ...
    from get_google_table(
      ...
      from (
      select replace(inn_old, '_', '') as inn_old, ogrn_old, replace(replace(a.inn, '_', ''),'nan','') as inn, replace(replace(ogrn,'nan',''),'.0','') ogrn
      from get_google_table('https://docs.google.com/spreadsheets/d/e/2PACX-1vRD4RejyrjcnZ0R7-wCzoRZZ6JrC17UsUrO3WfkuR8dqBb_vUK8c7c0W_0GRfiYXegs810IdF9iUy_8/pub?gid=948897933&single=true&output=csv'::text) a(inn_old text, ogrn_old text, inn text, ogrn text)
      ) a
